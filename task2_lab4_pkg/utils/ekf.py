...
class RobotEKF:
    def __init__(
        self,
        dim_x=1, #dimension of the state
        dim_u=1, #dimension of the imput (depends of the model used)
        eval_gux=None,
        eval_Gt=None,
        eval_Vt=None 
        #I dont say anything about the measurements since we can have multiple measurement functions (landmark model, liar model, imu)
        #We will define separately different measurement for each of the sensors and we will pass the functions of the measurements directly
        #to the update method (we will call update method for each of the sensor)
    ):
        """
        Initializes the extended Kalman filter creating the necessary matrices
        """
        self.mu = np.zeros((dim_x))  # mean state estimate (these will define the belief of my robot pose)
    ...
    def predict(self, u, sigma_u, g_extra_args=()):
        ...
        self.mu = self.eval_gux(self.mu, u, sigma_u, *g_extra_args) #eval_gux is one of the functions in probabilistic_model file
